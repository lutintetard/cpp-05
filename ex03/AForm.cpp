...
AForm::AForm(std::string const name, int const sign, int const exec, std::string target):
_name(name), _min_to_sign(sign), _min_to_exec(exec), target(target)
{
	if (sign < 1)
...
_min_to_sign(other._min_to_sign), _min_to_exec(other._min_to_exec), target(other.getTarget())
{
	this->_signed = false;
}
